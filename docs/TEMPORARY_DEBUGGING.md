...
- Behavior: Deletes the obsolete standalone Base only when it exactly matches Sprint's generated template. Restores the Tasks Base view only when that file exactly matches Sprint's boardless template. Customized files are preserved.
