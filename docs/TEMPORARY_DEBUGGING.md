...
- Behavior: Deletes the obsolete standalone Base only when it exactly matches Sprint's generated template. Restores the Tasks Base view or completed-task visibility only when the file exactly matches one of Sprint's earlier generated templates. Customized files are preserved.
