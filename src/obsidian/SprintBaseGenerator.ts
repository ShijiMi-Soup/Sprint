...
function sprintInstructions(settings: SprintSettings, profiles = settings.profiles): string {
  ...
  return [
    ...
    '- Human users should normally add future sprints with Sprint\'s Add Sprint or Generate future sprint controls.',
    '- AI agents may append a sprint note manually when the user asks them to or when they cannot invoke the UI. Follow the manual creation procedure below; never guess dates, reuse numbers, or overwrite an existing note.',
    ...
    '',
    'Use Sprint\'s controls when available. Otherwise, an agent may append future sprint notes directly as follows:',
    '',
    '1. Read the profile settings and every note in its `Sprints` folder. Confirm the configured duration, naming format, latest valid `start date`, and highest valid `sprint number`.',
    '2. Stop and ask the user if existing sprint numbers or dates conflict, the naming format is unclear, or the cadence cannot be established from settings and existing notes.',
    '3. Set the new number to the highest existing sprint number plus one. Build the filename from the configured naming format.',
    '4. Set the new start date to the latest dated sprint\'s start date plus the configured duration in weeks. Set the inclusive end date to `start date + (duration weeks * 7) - 1 day`. Use `YYYY-MM-DD` dates.',
    '5. Before writing, verify that the target path, sprint number, and start date are all unused. Never replace or renumber an existing sprint note.',
    '6. Create the note in the profile `Sprints` folder with numeric `sprint number`; date `start date` and `end date`; `sprint status: future`; and empty `review` and `retrospective` values.',
    '7. Include `## Goal`, `## Review`, and `## Retrospective` sections in the note body. Repeat the calculation from the newly created note when adding more than one sprint.',
    '8. Run Sprint synchronization when available so lifecycle statuses are normalized. If it cannot be run, leave the new note as `future` and tell the user that Sprint will reconcile it during the next synchronization.',
    '',
  ].join('\n');
}
