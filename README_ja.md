...
- プロジェクトごとのスイムレーンと、Backlog／スプリント列を備えたSprint Planner。
  タスク状態を保ったまま、プロジェクト間とスプリント間の両方へドラッグして移動可能
...
人が将来のSprintを追加するときはSprintの操作ボタンを使い、UIを操作できない
AIエージェントには安全な手動作成手順を使うよう、スキル内で案内しています。
