...
各プロジェクトと状態の列には**New task**操作があります。新しいタスクには、選択したプロジェクト、スプリント範囲、状態が自動的に設定されます。Estimateなど、設定したプロパティも入力できます。
...
Obsidianで**設定 -> コミュニティプラグイン -> 閲覧**を開き、作者が**shijimi-soup**の**Sprint**を検索して、インストールと有効化を行います。Sprintの**Options**を開き、**Automatic sprints**をオンにするとワークスペースが作成されます。
...
3. **Automatic sprints**をオンにして、ファイル作成の警告を確認します。
